...
void bluefruit_pins_init(void) {
    
    /* 
    * Extra pins used by bluefruit EzKey
    */
    
    /* Battery LED: output / low (LED off) */
//    DDR_BAT_LED |= (1<<BIT_BAT_LED);
//    PORT_BAT_LED &= ~(1<<BIT_BAT_LED);
...
//    PORT_BF_RESET &= ~(1<<BIT_BF_RESET);
    ...
    /* Battery Voltage sensor: initial Input / Hi-Z */
//    DDR_BAT_VOLT &= ~(1<<BIT_BAT_VOLT);
//    PORT_BAT_VOLT &= ~(1<<BIT_BAT_VOLT);
