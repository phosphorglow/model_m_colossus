...
const uint8_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
     ...
     */
    KEYMAP( \
        ESC,        F1,  F2,  F3,  F4,  F5,  F6,  F7,  F8,   F9,   F10,   F11,   F12,         PSCR,  SLCK, FN2, \
...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt)
{
	...
	return (record->event.pressed ?
      ...
      case TEST:
	return (record->event.pressed ?
	    MACRO( I(5), D(LSHIFT), T(I), U(LSHIFT), T(N), T(V), T(O), T(I), T(C), T(E), T(SPC), T(T), T(O), D(LSHIFT), T(SCLN), U(LSHIFT), T(SPC), T(SPC), D(LCTRL), D(V), U(LCTRL), U(V), T(DOT), END ) :
   	    MACRO_NONE );      
